...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        operatorOfExp = scheme_eval(first, env) #evals the first element in the expr list, stored in env
        check_procedure(operatorOfExp) #checks if eval on op returns a procedure
            #raise SchemeError('{}'.format(first, env)) #still don't know this syntax lol
        return scheme_apply(operatorOfExp, rest.map(lambda eachOperand: scheme_eval(eachOperand, env)), env) #apply evaluated operator on each operand
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        if symbol in self.bindings: #search for symbol in curr frame (dict)
            return self.bindings[symbol] #return the value of that symbol key in dict
        if self.parent: #if the frame has a parent
            return self.parent.lookup(symbol) #call lookup in parent frame on symbol
        # END PROBLEM 3
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: #args to be passed in in *args format...how to do??
            return self.fn(*python_args)
        except: #how to format general scheme error??
            raise SchemeError('{}'.format(python_args))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        definedValue = scheme_eval(expressions.second.first, env) #evaluate the actual expression part inputted after the var to be defined
        env.define(target, definedValue) #binds the target to the definedValue in the current environment
        return target #returns the name of the newly defined var
        # END PROBLEM 6
...
def do_quote_form(expressions, env):
  ...
  #  to_print = []
   # if expressions is Link.empty:
      #  return scheme_read(to_print)
    #expressions = expressions.rest
   # to_print.append(expressions.first) 
